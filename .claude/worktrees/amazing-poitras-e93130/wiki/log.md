...
## [2026-04-18] edit | Core data-domain concepts + procedures + wiki-v2 query UI
- Pages created:
  - `concepts/census` — employee-demographic master; EE_BEN_ADDRESS join; gap-audit fields
  - `concepts/investment-elections` — elections by source, QDIA default, Save Express / E-Statement / secure-data siblings
  - `concepts/roth` — Roth money source, basis buckets, 5-year clock / RTH_CTRB_FST_YR
  - `concepts/basis` — cost-basis buckets (pre-87 / post-86 / Roth / Type 3/6 / deemed-loan); EE_BASIS
  - `concepts/loans` — loan headers + per-source splits + repayments + HOLB; IRS 50%/$50K mechanics
  - `concepts/awd` — full AWD concept: work types, queues, Prod Support / FILESPECRQ / Reversal / Rebook tickets
  - `processes/census-load` — inbound census → EDS → gap audits → QA sign-off; failure modes
  - `processes/investment-elections-load` — 4-part migration: fund elections, Save Express, E-Statement, secure-data
  - `processes/loan-takeover` — pre-liquidation snapshot, HOLB, takeover-loan header + source queries, Informatica Loan Module, QA verification
  - `processes/prod-support-ticket` — AWD Create Work flow; isolating query + end-state template; verify-and-close
- Pages updated: `index` — pages 107→117; added Processes rows and new "Concepts — Core Data Domains" subsection
- Source material: wiki-v2.html query library (`to-census`, `current-elections`, `current-elections-multi-case`, `basis-internal-conversion`, `to-basis`, `takeover-loan-query-build`, `takeover-loan-sources`, `loan-balances-prior-to-liquidation`, `highest-loan-balance`, `saveexpress-copy`, `estatement-copy`, `email-copy`, `new-deferral-report`, `excelwise-default-election-check`), plus existing `entities/prod-support`, `glossary/awd`, `processes/reversal-submission`, `sources/dc-brain-dump-001`, `sources/lmdc-training-notebook`, `sources/extra-questions-001`
- All 10 new pages carry a `> [!needs-review]` callout and an `Open Questions` section — human flagged these for close review on the next lint pass; human has additional material to contribute per-topic
- Also planned: wiki-v2.html query-page UI — parameter inputs (reset each visit) + copy-substituted-SQL button in top-right of each code block (separate edit)

## [2026-04-18] edit | Balance Import concept + 3 procedure pages; remove DB Plan Setup + Incentive Compensation
- Pages created:
  - `concepts/balance-import` — hub concept: 3 types (cash/mapping/TIK), CIT balance file structure, Informatica mp_part_balance map, P3 processing modes, trade-once rule
  - `processes/balance-import-cash` — AE liquidation request → CIT file → Informatica test/prod → Process Immediate + Batch → ROC dummy reversal → verify
  - `processes/balance-import-mapping` — CIT file → Informatica test/prod → Online + No Hold (trades already out on Day of Wire) → ROC dummy reversal → verify
  - `processes/balance-import-tik` — update Matt O'Connell tracker → CIT file → Informatica test/prod (fills Bill Remit detail) → Online + No Hold → verify
- Pages removed from wiki-v2.html: `processes/db-plan-setup`, `processes/incentive-compensation`
- Source material: `sources/lmdc-training-notebook` (Informatica Balance Module parameters, mp_part_balance map), `sources/dc-brain-dump-001` (final files sequence, P3 modes by conversion type), `sources/extra-questions-001` (CIT balance file structure, trade-once rule), `sources/lmdc-training-notebook` (AE liquidation, wire receipt workflow)
- All 4 new pages carry `> [!needs-review]` — flagged for SME verification on next lint pass
- wiki-v2.html: page count 205 → 207 (net: +4 added, −2 removed)
