...
Never modify files in `raw/`. Those are immutable source documents. After ingesting a source, move it from `raw/pending/` to `raw/ingested/`.
...
├── apps/                      ← HTML tools: checklist generators, trackers
├── archive/                   ← old or uncertain files kept just in case
├── docs/                      ← project-level reference docs (design systems, etc.)
├── personal/                  ← files unrelated to TA Brain (do not touch)
├── presentations/             ← Marp slideshows derived from wiki content
├── scripts/                   ← automation: build-wiki.py, gmail-bridge.gs, SETUP.md
├── raw/                       ← immutable source documents (human adds these)
│   ├── assets/                ← locally downloaded images
│   ├── ingested/              ← sources already processed into wiki
│   └── pending/               ← sources dropped in, not yet ingested
└── wiki/                      ← LLM-owned knowledge base
    ...
    │   ├── systems/           ← internal tools and platforms
    │   └── teams/             ← internal organizational units
    └── concepts/
        ├── process-steps/     ← discrete steps in a workflow
        ├── document-types/    ← forms, files, packages, outputs
        ├── system-procedures/ ← EDS, Informatica, FTP how-tos
        └── plan-setup/        ← plan configuration concepts
```

**Folder guide:**
- `apps/` — standalone HTML tools built for TA Brain users; do not modify without testing
- `raw/pending/` — source files waiting to be ingested; after ingesting, move to `raw/ingested/`
...
- `entities/people/` — named individuals with role in the conversion process
- `entities/systems/` — internal tools and platforms (NBI, P3, EDS, Informatica, etc.)
- `entities/teams/` — internal organizational units (Cashiering, FTP Team, etc.)
- `concepts/process-steps/` — discrete workflow actions (liquidation day, early access, etc.)
- `concepts/document-types/` — forms and file types (PRD, RDD, subpack, CCCS, etc.)
- `concepts/system-procedures/` — step-by-step how-tos for EDS and Informatica operations
- `concepts/plan-setup/` — plan configuration concepts (fund mapping, conversion types, etc.)
...
Trigger: human drops a file in `raw/pending/` and says "ingest [filename]" or similar. Files may also come directly from `raw/ingested/` if already moved manually.
...
13. **Move** the source file from `raw/pending/` to `raw/ingested/` (if it was in pending).
14. **Append** to `wiki/log.md` — one entry with the format below.
...
| Page | Summary | Sources |
|------|---------|---------|
| [[entities/people/slug]] | One-line summary | N |

## Entities — Systems
| Page | Summary | Sources |
|------|---------|---------|
| [[entities/systems/slug]] | One-line summary | N |

## Entities — Teams
| Page | Summary | Sources |
|------|---------|---------|
| [[entities/teams/slug]] | One-line summary | N |

## Concepts — Process Steps
| Page | Summary | Sources |
|------|---------|---------|
| [[concepts/process-steps/slug]] | One-line summary | N |

## Concepts — Document Types
| Page | Summary | Sources |
|------|---------|---------|
| [[concepts/document-types/slug]] | One-line summary | N |

## Concepts — System Procedures
| Page | Summary | Sources |
|------|---------|---------|
| [[concepts/system-procedures/slug]] | One-line summary | N |

## Concepts — Plan Setup
| Page | Summary | Sources |
|------|---------|---------|
| [[concepts/plan-setup/slug]] | One-line summary | N |
