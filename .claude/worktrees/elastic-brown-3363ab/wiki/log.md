...
## [2026-04-18] edit | Full repository reorganization
- Goal: separate the knowledge product (wiki), tooling (scripts, apps), source material (raw), and unrelated files into clearly distinct folder layers.
- Root-level folders created: `apps/`, `presentations/`, `personal/`, `archive/`, `docs/`
- Files moved to `apps/`: App - Main Checklist Generator v2.html, App - Main Checklist Generator.html, COM Main Checklist.html, DC Onboarding Tracker.html
- Files moved to `presentations/`: dc-onboarding.md/.html, lm-dc-conversion-timeline.md/.html (Marp slideshows derived from wiki content)
- Files moved to `personal/`: ai-investment-pitch.md/.html, app-safety-presentation.md/.pdf (unrelated to TA Brain)
- Files moved to `archive/`: Balance-Import-Guide.md, Balance-Import-Guide-Apple.md/.html (old/uncertain, kept just in case)
- Files moved to `docs/`: DESIGN.md → docs/design-cursor.md, apple/DESIGN.md → docs/design-apple.md (design system references)
- Files moved to `scripts/`: build-wiki.py
- `raw/` restructured: `raw/ingested/` (LMDC.mht, extra-questions-001.md) and `raw/pending/` (loading-eligibility-through-eds.txt, sub-pack-raw-cccs.txt); 12 URL-encoded title-only stub files deleted; test file deleted; old `raw/new mds/` folder removed
- `wiki/entities/` split into `wiki/entities/people/` (10 pages), `wiki/entities/systems/` (9 pages), `wiki/entities/teams/` (4 pages)
- `wiki/concepts/` split into 4 subfolders: `process-steps/` (8 pages), `document-types/` (18 pages), `system-procedures/` (13 pages), `plan-setup/` (14 pages)
- Empty junk files deleted: 2026-04-14.md, nbi.md (root), payroll-support.md (root); broken gitlink `cinematic-site-components` removed
- `CLAUDE.md` updated to reflect all structural changes, including raw/pending → raw/ingested workflow for ingest operations
- Notes: Obsidian [[links]] unaffected — filenames unchanged, only folder depth changed. Submodule `cinematic-site-components` was a broken gitlink (no .gitmodules) pointing to an external commit; removed as non-functional.
