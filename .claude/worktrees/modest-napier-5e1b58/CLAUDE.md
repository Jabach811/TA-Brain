...
# File Handling & Versioning Workflow

## Core Rule: Never Overwrite
- Always create a copy before making any changes.
- Do **not** overwrite the original file under any circumstances.
- This is a strict rule due to prior data loss issues.

## Working Directory Structure
- Maintain a single **master working folder**.
- This folder will contain:
  - The **main working file**
  - All **versioned copies**
- This serves as the central location for all iterations and tracking.

## Versioning Approach
- Every change results in a new copy.
- Copies remain in the same working directory for visibility and control.
- Cleanup of old versions will happen later manually.

## Handling Small Changes
- Even for minor edits (spacing, font size, small tweaks), still create a copy.
- However, if the change is clearly insignificant:
  - Prompt before proceeding:
    - _"This looks like a minor change—do you want me to skip keeping this extra copy?"_
- Allow decision-making before unnecessary version clutter.

## File Organization Behavior
- If files are placed incorrectly (e.g., dropped into the landing page instead of proper folders):
  - After processing/ingestion:
    - Prompt:
      - _"Do you want me to move these files to their proper folders?"_
  - If confirmed:
    - Move files to appropriate directories such as:
      - `/app`
      - `/raw`
      - `/data`
      - or other relevant folders

## Animation & Design File Handling
- When animations are requested:
  - Use the designated **animation skill**
  - Follow the same structure and conventions as other assets
- Maintain consistency in placement, naming, and integration

## General Behavior Expectations
- Be proactive but controlled:
  - Always create copies
  - Ask before skipping copies for minor changes
  - Ask before reorganizing files
- Maintain clean structure while preserving full version history
- Keep everything centralized and traceable
