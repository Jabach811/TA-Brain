...
## [2026-04-18] lint | Wiki health check — 5 issues fixed
- Pages created: none
- Pages updated (4):
  - `entities/eric-leyten` → renamed to `entities/eric-leytem` — filename was wrong; title/content already said "Eric Leytem"; log entry from 2026-04-14 had documented the name correction but the file was never renamed
  - `onboarding/lm-dc` — fixed broken `[[eric-leyten]]` → `[[eric-leytem]]`
  - `concepts/subpack` — added `[[roles/com]]` and `[[roles/tc]]` to See Also (missing despite COM and TC being the two primary owners of this document)
  - `concepts/fund-management-calendar` — added `[[rich-lippoth]]` and `[[ssbt]]` to See Also (Rich Lippoth is the Vanguard approval contact named on this page; SSBT cut-off is the defining deadline)
  - `index` — corrected page count 107 → 103; updated date to 2026-04-18
- Issues resolved: broken filename (1), broken wiki link (1), index count wrong (1), missing See Also cross-refs (2 pages, 4 links)
- Issues flagged but not fixed:
  - **Log order anomaly**: The "Wiki initialized" entry (2026-04-14) appears at the bottom of the log rather than at the top; log says "most recent entries last" but initialized entry was appended out of order; left in place as it is append-only
  - **12 stubs with sources=0**: All are structured stubs with hub references and context; not empty — waiting on source documents to be ingested
  - **Open data gaps**: Samantha C. last name; Paul/Danny (QA) last names; prior RK-specific workflow templates; DC query library; WF templates in DC folder
- Notes: No orphan pages found. All 9 roles/teams in internal-teams have a corresponding wiki page. No active contradiction callouts present.
