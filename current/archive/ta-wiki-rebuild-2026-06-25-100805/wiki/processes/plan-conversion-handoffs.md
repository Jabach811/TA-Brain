...
This page documents the handoffs â€” what moves, between whom, and when.

## Trigger

Two parallel triggers start a conversion:
- **COM trigger:** [[ellen-miller]] (or sales) issues an [[nsa]] (New Sale Announcement) â†’ COM receives it â†’ COM creates project plan and schedules kickoffs
- **DC trigger:** [[ellen-miller]] notifies a manager that a plan slot is open in NBI â†’ manager assigns a DC â†’ DC enters active pipeline
...
- **From:** [[lm-dc]] â†’ [[matt-oconnell]]
- **Trigger:** TOA reviewed; any fund with column T = Y
- **What moves:** Re-registration form (DC fills out)
- **Timing:** Target ~10â€“12 business days before liquidation
- **Risk:** Too early = premature action; too late = liquidation can't proceed

### 6. Liquidation Day â€” Cashiering Loop
- **From:** [[lm-dc]] â†’ [[cashiering]] (heads-up, day before)
- **From:** Prior record keeper â†’ wire â†’ [[cashiering]]
- **From:** [[cashiering]] â†’ [[lm-dc]] (wire receipt + ref number)
...
### 7. Eligibility Gate
- **From:** [[lm-dc]] â†’ [[qa]] + [[tc]] + [[com]] (notification)
...
- **From:** [[lm-dc]] â†’ [[doc-ops]]
- **Trigger:** Plan requires a new special class not already in P3
- **What moves:** Request (presumably via ticket or email â€” method not yet documented)
...
### 10. NSA â†’ COM Kickoff
- **From:** Sales / business development â†’ [[com]]
...
- **From:** Plan Services â†’ [[com]] â†’ [[ellen-miller]] â†’ docs team
...
- **From:** [[com]] â†’ plan sponsor + prior carrier
...
- **From:** [[com]] â†’ plan sponsor (approval) â†’ [[ellen-miller]] (mailing)
...
- **From:** [[com]] â†’ client â†’ [[com]] â†’ [[lm-dc]]
...
- **From:** [[com]] â†’ client â†’ [[com]] â†’ [[tc]]
- **Trigger:** TFS plans requiring contribution monitoring
- **What moves:** [[contribution-monitoring-form]]; TC AWD setting change
- **Risk:** If COM doesn't follow up on the TC confirmation, the AWD setting may not be flipped â€” monitoring doesn't activate at go-live

### 16. RDD Form Loop
- **From:** [[tc]] â†’ [[com]] â†’ client â†’ [[com]] â†’ [[tc]]
- **Trigger:** Payroll setup phase
- **What moves:** [[rdd-form]]
- **Risk:** Client slow to return; COM must track and follow up; TC cannot proceed without it

### 17. Early Access Activation
- **From:** [[com]] â†’ [[lm-dc]] (template request) â†’ client (file completion) â†’ [[lm-dc]] (file load) â†’ [[tc]] (activation)
...
- **Risk:** Client returns file late â†’ TC cannot be notified in time â†’ early access cannot open as scheduled

### 18. Assets Received Notification
- **From:** [[lm-dc]] â†’ [[com]] â†’ client
- **Trigger:** Liquidation day â€” DC confirms assets received and deposited
...
### 19. CSR Delivery
- **From:** Quality Consultant â†’ [[com]] â†’ client
- **Trigger:** Go-live
...
- **From:** [[com]] â†’ internal team + client + [[ellen-miller]] + [[nbi]]
...
- **From:** [[lm-dc]] â†’ Fiduciary Services; [[com]] â†’ Account Manager
...
### 22. Source Mapping Attribute Completion
- **From:** [[lm-dc]] -> [[tc]] -> COM/TM
- **Trigger:** DC identifies all prior-vendor sources with balances and saves the working spreadsheet
- **What moves:** Source list, balances, prior-plan review attributes, contribution type codes, contribution frequency, withdrawal provisions, vesting, and unresolved frozen-source questions
- **TC action:** Reviews the prior plan material, completes administrative attributes, asks DC to obtain missing prior-vendor details, and routes completed mapping information back to the project owner
- **Risk:** Frozen or legacy sources get mapped without verified attributes; downstream setup and participant rules become wrong
- **See:** [[source-mapping-tc]]

### 23. TC Administrative Readiness Gate
- **From:** [[tc]] + [[qa]] -> conversion team -> ongoing team
- **Trigger:** Limited access, full access, or conversion closeout is approaching
- **What moves:** Case notes, system audit evidence, DDOL/VRU/PSD access status, remittance/admin guidance, special-service setup, participant website audit results, and final subpack
- **TC action:** Confirms administrative procedures are documented and that participant/sponsor access is ready before activation
- **QA action:** Validates setup and documents audit/control findings
- **Risk:** The plan goes live with incomplete case notes, wrong access settings, missing special-service setup, or weak handoff to the ongoing team
- **See:** [[tc-conversion-timeline]], [[go-live-checklist]], [[case-notes-template]]
## Failure Modes
...
| Subpack | COM creates it but TC must take over after review â€” if handoff never happens, subpack is orphaned |
...
- [[dc-onboarding-workflow]] â€” DC's view of the full 7-phase process
- [[lm-dc]] â€” DC role reference
- [[com]] â€” COM role reference (full 18-phase checklist breakdown)
- [[liquidation-day]] â€” detail on the Day of Wire sequence
- [[eligibility-and-deferrals]] â€” the eligibility gate detail
- [[nsa]] â€” the trigger that starts COM's workflow
- [[blackout-notice]] â€” legally required participant communication
- [[early-access]] â€” pre-go-live participant window
- [[csr]] â€” Quality Consultant's go-live deliverable
- [[payroll-handoff-at-go-live]] â€” new procedure (Feb 26, 2026) when payroll testing is incomplete at go-live
